...
export default function ParkDetail() {
      ...
      <div className="min-h-full bg-cream">
        <header className="border-b border-cream-dark bg-white/80 backdrop-blur-sm sticky top-0 z-10">
          <div className="max-w-7xl mx-auto px-4 lg:px-8 py-3 flex items-center gap-4">
            ...
            <span className="font-display text-lg text-charcoal">Woodsmoke</span>
          </div>
        </header>
        <main className="max-w-4xl mx-auto px-4 py-16 text-center">
          <h1 className="font-display text-3xl text-charcoal mb-4">Park not found</h1>
  ...
  return (
    <div className="min-h-full bg-cream">
      ...
      <main className="max-w-7xl mx-auto px-4 lg:px-8 lg:h-[calc(100vh-49px)] lg:overflow-hidden">
        <div className="lg:grid lg:grid-cols-5 lg:gap-12 pt-6 lg:pt-8 lg:h-full">
          ...
          <div className="lg:col-span-3 space-y-6 lg:overflow-y-auto lg:pb-8">
            ...
            <div>
              <p className="text-charcoal leading-relaxed text-[15px]">{park.description}</p>
            ...
            {park.activities.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {park.activities.map((activity) => (
                  <span
                    key={activity}
                    className="px-3 py-1.5 rounded-full bg-sage/10 text-sage-dark text-sm font-medium"
                  >
          ...
          <aside className="mt-10 lg:mt-0 lg:col-span-2 lg:overflow-y-auto lg:pl-8 lg:border-l lg:border-sage/15">
            <div className="space-y-6 lg:pb-6">

              {/* Mobile section heading */}
              <h2 className="font-display text-xl text-charcoal lg:hidden flex items-center gap-2 border-t border-sage/15 pt-6">
              ...
              {park.campgrounds.length > 0 && (
                <div id="campground-section" className="scroll-mt-16">
                  ...
                  {park.campgrounds.length === 1 ? (
                    <p className="text-sm font-medium text-charcoal">{park.campgrounds[0].name}</p>
                  ) : park.campgrounds.length <= 3 ? (
                    <div className="flex rounded-xl bg-cream-dark/50 p-1">
                      {park.campgrounds.map((campground, i) => (
                        <button
                          key={campground.name}
                          type="button"
                          onClick={() => setSelectedCampground(i)}
                          className={`flex-1 py-2 px-3 rounded-lg text-xs font-medium transition-all truncate ${
                    ...
                    <select
                      ...
                      className="w-full rounded-xl border border-cream-dark bg-white px-3 py-2 text-sm text-charcoal"
                    >
                      {park.campgrounds.map((campground, i) => (
                  ...
                  {cg && (
                    <div className="mt-3 space-y-2">
                      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-charcoal-light">
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-sm border-t border-cream-dark z-20">
        <div className="flex justify-around py-2 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
          <button
            type="button"
            onClick={() => scrollToSection('weather-section')}
            className="flex flex-col items-center gap-0.5 text-charcoal-light hover:text-sage transition-colors px-4 py-1"
          >
